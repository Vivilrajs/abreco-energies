// Seed solutions + default settings. Run: node scripts/seed.mjs
...
const solutions = [
  {
    ...
    order: 1,
    published: true,
    description: "Efficient heat pumps that keep your home comfortable year-round.",
  ...
  },
  {
    ...
    order: 2,
    published: true,
    description: "Eco-friendly air conditioners that save energy while enhancing comfort.",
  ...
  },
  {
    ...
    order: 4,
    published: true,
    description: "Reliable commercial solar systems that reduce costs and support the environment.",
...
async function run() {
  ...
  for (const s of solutions) {
    await Solution.updateOne({ slug: s.slug }, { $set: s }, { upsert: true });
  }
  ...
  console.log("Seeded", solutions.length, "solutions + settings.");
